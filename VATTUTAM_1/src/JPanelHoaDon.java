...
//    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if(e.getSource() == btnaddHD){
            ...
            long vat = Long.parseLong(txtvat.getText());
            long donGia = Long.parseLong(txtdongia.getText());
            modelHoaDon.addRow(new Object[]{tbhoadon.getRowCount() + 1, vatTuValueSelected, vatTuNameSelected, soLuong, donGia,
                    vat, ChuanHoa.ChuyenSoThanhTien(String.valueOf((long)(
...
//    }
    ...
    public void CapNhatMatHang(){
        ...
        if(choose == 0){
            System.out.println("dang chon:" + tbhoadon.getSelectedRow());
            long soLuong=Long.parseLong(txtsoluong.getText());
            long vat = Long.parseLong(txtvat.getText());
            long donGia = Long.parseLong(txtdongia.getText());
            for (int i = 0; i < tbhoadon.getRowCount(); i++) {
                if (tbhoadon.getValueAt(i, 1).toString().equals(mavtchoosed.getText())){
                    if((tbvattu.getSelectedRow())==i || tbvattu.getSelectedRow()==0 && i==0){
                        soLuongMatHang = String.valueOf(soLuong);
                        donGiaMatHang = String.valueOf(donGia);
                        vatMatHang = String.valueOf(vat);
                        System.out.println("TRUNG MA TA");
                    }
                    System.out.println("vi tri:" + i);
                    tbhoadon.setValueAt(String.valueOf(soLuong), i, 3);
                    tbhoadon.setValueAt(String.valueOf(donGia), i, 4);
                    tbhoadon.setValueAt(String.valueOf(vat), i, 5);
